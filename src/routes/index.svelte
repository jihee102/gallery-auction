<script>
	...
	const searchCondition = async (e) => {
		...
		if(priceValue !=='none'){
			switch (priceValue) {
				...
				case"lToh":
					...
					break;
				case "less":
					filterPainting = filterPainting.filter(paint => Number(paint.value) < 60000000);
					break;
				case "more":
					filterPainting = filterPainting.filter(paint => Number(paint.value) >= 60000000);
					break;
		...
		if(sizeValue !=='none'){
			switch (sizeValue) {
				case "less5":
					filterPainting = filterPainting.filter(paint => Number(paint.size) <5);
					break;
				case "more5":
					filterPainting = filterPainting.filter(paint=> Number(paint.size)>=5);
					break;
		...
		if(nameValue!=='none'){
			switch (nameValue) {
				...
				case "az":
					filterPainting = filterPainting.sort(((a, b) => a.name.localeCompare(b.name)));
					break;
					// descending alphabetical order
				case "za":
					filterPainting =  filterPainting.sort(((a, b) => b.name.localeCompare(a.name)));
					break;
			}
		}
