<script>
  import { tokenInfo } from '../auctionStorage.js';
  ...
  const tyrLogin = async () => {
    const response = await fetch(
      'https://gallery-auction.herokuapp.com/user/log-in',
      {
        method: 'POST',
        headers: {
...
<form class="login_form">
  ...
  <input
    ...
    bind:value={username}
    class={username !== undefined && username.length < 3 ? 'bad' : ''}
  />
  ...
  <input
    ...
    bind:value={password}
    class={password !== undefined && password.length < 3 ? 'bad' : ''}
  />
